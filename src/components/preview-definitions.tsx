...
import { GroupedDefinition, WordData } from '../models';
  ...
  return (
    <div>
      <div className="flex flex-row items-center mb-4">
        ...
        {audio && (
          <div className="w-4 h-4 cursor-pointer" onClick={() => new Audio(audio).play()}>
            🔊
          </div>
        )}
      </div>
            ...
            {meanings.map((meaning) =>
              meaning.definitions.map((def, idx) => (
                <div className="mb-2">
                  ...
                  <span className="font-medium">{def.definition.replace(/\.$/, '')}</span>
